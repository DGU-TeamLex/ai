...
제품 백엔드 기능은
[DGU-TeamLex/backend](https://github.com/DGU-TeamLex/backend)가 담당합니다.
...
src/loading/                    검증된 AI 배치 산출물 계산·보호된 DB 적재·backend handoff
...
`dev`에서 운영 중인 재고 보조값 계산·반영 도구도 함께 유지합니다.

| 도구 | 역할 |
|---|---|
| `src/loading/compute_mu_forecast.py` | 최근 3개월 일수요율 기반 `mu_forecast` 산출 |
| `src/loading/backtest_holdout.py` | `mu_forecast` 홀드아웃 검증 |
| `src/loading/compute_zero_stock_reason.py` | 미운영·데이터누락·실제결품 분류 |
| `src/loading/apply_mu_forecast.py` | 검증된 `mu_forecast` 반영 |
| `src/loading/apply_zero_stock_reason.py` | 재고 0 원인 반영 |
| `src/loading/apply_family_stock.py` | family 단위 재고 집계 반영 |
| `src/loading/apply_is_medical.py` | 비의료품 3단계 분류 반영 |
| `src/loading/apply_supply_risk_policy.py` | 공급위험 기준레벨 정책 반영 |

DB 스키마 변경은 backend 책임이다. 반영 도구는 `DRY_RUN`과 대상 DB, 매칭률,
입력 handoff를 확인한 뒤 실행하고, AI 예측 코드가 직접 DDL을 수행하지 않는다.
...
data/handoff/mu_forecast.csv
data/handoff/zero_stock_reason.csv
data/handoff/supply_risk_level_policy.json
