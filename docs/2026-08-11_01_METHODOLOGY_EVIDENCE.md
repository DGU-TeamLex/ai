...
**결정** 탐색적으로 돌린 가격→재고 검정 90건에는 Bonferroni 보정을 적용하고,
**기관이 사전지정한 PP 는 별도로 취급**한다.
...
**적용**
- 탐색 90건: 유의 14건, 그러나 **두 설계를 모두 통과한 것은 0건**
- PP(사전지정): 관세청 수입가 → 기말재고, lag 2개월, p=0.007. lag 1~4 전부 유의하고
  부호 일관. PP 내부 8건 기준 보정 p = 0.007 × 8 = **0.056**

**해석** PP 는 경계선이다. 0.05 를 아슬하게 넘는다. "유의하다" 고 단정하지 않고
"사전지정 가설로서 시사적, 표본 확대 후 재확인" 으로 기술한다.
